...
Afin d'accéder au dashboard du cloud storage, il vous suffira d'accéder à l'url suivante : `https://127.0.0.1:8000` et d'y renseigner l'identifiant et le mot de passe précédemment renseigner dans le .env.local dans la partie AWS.
...
Une fois le serveur lancé, la partie utilisateur se lancera sur l'url suivante : `https://127.0.0.1:8000`.
N'importe qui pourra upload des images sans devoir être connecté.

Afin d'accéder à la partie back-office, il vous suffira d'accéder à l'url suivante : `https://127.0.0.1:8000/back-office` et de vous créer un compte si vous n'en avez pas encore en cliquant sur le lien 'S'inscrire'.
...
Si vous avez déjà un compte existant, en accédant à l'url suivant : `https://127.0.0.1:8000/back-office`, il vous suffira de vous connecter et vous serez redirigé automatiquement vers le back-office.
